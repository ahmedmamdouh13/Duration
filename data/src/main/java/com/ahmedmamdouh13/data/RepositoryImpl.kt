...
class RepositoryImpl(
        ...
        return try {
            ...
            if (await.isSuccessful) {
                ...
                val list = holidays!!.map {
                    ...
                    HolidaysDomain(
                        it.name,
                        DateTypeConverter().run { toString(it.date) },
                        it.locations,
                        it.description ?: ""
                    )
    ...
    override suspend fun getTasks(id: String): MyResult<List<TaskDomain>> {
        return try {
    ...
    override suspend fun getProjectById(id: Int): MyResult<ProjectDomainModel> {
        return try {
            val entity = projDao.getProjectById(id)
            MyResult(ProjectDomainModel(entity.key, entity.title, entity.startDate, entity.endDate))
        } catch (e: Exception) {
            MyResult(ProjectDomainModel(-1, "", "", ""))
                .apply {
                    status = Status.ERROR
